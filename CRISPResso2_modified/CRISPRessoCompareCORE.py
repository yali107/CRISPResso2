...
def get_amplicon_output(amplicon_name, output_folder):
    ...
    if os.path.exists(quantification_file) and profile_file:
        ...
        raise CRISPRessoShared.OutputFolderIncompleteException(
            'The folder %s is not a valid CRISPResso2 output folder. Cannot find profile file %s for amplicon %s.' % (
            output_folder, profile_file, amplicon_name))
...
|___________________________|
        ...
        parser = argparse.ArgumentParser(description='CRISPRessoCompare Parameters',
                                         formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser.add_argument('crispresso_output_folder_1', type=str, help='First output folder with CRISPResso analysis')
        parser.add_argument('crispresso_output_folder_2', type=str,
        ...
        if args.sample_1_name is None:
            ...
            if 'running_info' in run_info_1 and 'name' in run_info_1['running_info'] and run_info_1['running_info'][
                'name']:
        ...
        if args.sample_2_name is None:
            ...
            if 'running_info' in run_info_2 and 'name' in run_info_2['running_info'] and run_info_2['running_info'][
                'name']:
        ...
        if not args.name:
            database_id = '%s_VS_%s' % (get_name_from_folder(args.crispresso_output_folder_1),
        ...
        _jp = lambda filename: os.path.join(OUTPUT_DIRECTORY,
                                            filename)  # handy function to put a file in the output directory
        ...
        # LOAD DATA
        amplicon_names_in_both = [amplicon_name for amplicon_name in amplicon_names_1 if
                                  amplicon_name in amplicon_names_2]
        ...
        for amplicon_name in amplicon_names_in_both:
            ...
            if os.path.isfile(output_1):
                crispresso2_info['results']['general_plots']['summary_plot_datas'][plot_name].append(
                    (sample_1_name + ' output', os.path.relpath(output_1, OUTPUT_DIRECTORY)))
            if os.path.isfile(output_2):
                crispresso2_info['results']['general_plots']['summary_plot_datas'][plot_name].append(
                    (sample_2_name + ' output', os.path.relpath(output_2, OUTPUT_DIRECTORY)))
            ...
            for mod in ['Insertions', 'Deletions', 'Substitutions', 'All_modifications']:
                ...
                fisher_results = [stats.fisher_exact([[z[0], z[1]], [z[2], z[3]]]) if max(z) > 0 else [np.NaN, 1.0] for
                                  z in zip(mod_counts_1, unmod_counts_1, mod_counts_2, unmod_counts_2)]
                ...
                crispresso2_info['results']['general_plots']['summary_plot_datas'][plot_name] = [
                    (mod_name + ' quantification', os.path.basename(mod_filename))]
                ...
                for allele_file_2 in allele_files_2:
                    ...
                    if allele_file_1_name == allele_file_2_name:
                        ...
                        if len(df1.loc[df1['Reference_Sequence'].str.contains('-') == False]) > 0:
                            ref_seq_around_cut = \
                            df1.loc[df1['Reference_Sequence'].str.contains('-') == False]['Reference_Sequence'].iloc[0]
                        # otherwise figure out which sgRNA was used for this comparison
                            ...
                            ref_seq_around_cut = \
                            df2.loc[df2['Reference_Sequence'].str.contains('-') == False]['Reference_Sequence'].iloc[0]
                        else:
                            ...
                            for sgRNA_interval, cut_point in zip(sgRNA_intervals, cut_points):
                                ...
                                if sgRNA_seq in allele_file_1_name:
                                    ...
                                    ref_seq_around_cut = consensus_sequence[max(0,
                                                                                this_cut_point - args.offset_around_cut_to_plot + 1):min(
                                        len(reference_seq), cut_point + args.offset_around_cut_to_plot + 1)]
                                    break

                        merged = pd.merge(df1, df2,
                                          on=['Aligned_Sequence', 'Reference_Sequence', 'Unedited', 'n_deleted',
                                              'n_inserted', 'n_mutated'],
                                          suffixes=('_' + sample_1_name, '_' + sample_2_name), how='outer')
                        quant_cols = ['#Reads_' + sample_1_name, '%Reads_' + sample_1_name, '#Reads_' + sample_2_name,
                                      '%Reads_' + sample_2_name]
                        merged[quant_cols] = merged[quant_cols].fillna(0)
                        lfc_error = 0.1
                        merged['each_LFC'] = np.log2(((merged['%Reads_' + sample_1_name] + lfc_error) / (
                                    merged['%Reads_' + sample_2_name] + lfc_error)).astype(float)).replace(
                            [np.inf, np.NaN], 0)
                        ...
                        CRISPRessoPlot.plot_alleles_table_compare(ref_seq_around_cut,
                                                                  merged.sort_values(['each_LFC'], ascending=True),
                                                                  sample_1_name, sample_2_name, _jp(plot_name),
